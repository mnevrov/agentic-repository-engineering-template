...
> **Presenter guide.** Перед выступлением проверьте [`STATUS.md`](STATUS.md). Этот сценарий можно использовать как структуру рассказа до полного demo dry-run, но он не превращает непроверенные checkpoints/demo в подтверждённый результат.
