...
> **Presenter/Maintainer runbook.** Это обязательный gate перед фиксацией meetup-версии. Текущий факт готовности хранится только в [`STATUS.md`](STATUS.md). Само наличие этого runbook, template CI или unit tests **не означает**, что презентационный demo уже проверен.
...
# 46. Что сделать после успешного прогона

Если полный инженерный dry-run и презентационная репетиция действительно прошли:

1. сохраните фактические результаты/заметки репетиции;
2. убедитесь, что critical rows из раздела 36 имеют PASS/evidence;
3. обновите [`STATUS.md`](STATUS.md) с `DRY-RUN NOT COMPLETED` на `VERIFIED FOR MEETUP`;
4. укажите exact demo repository/checkpoints, которые реально использовались;
5. повторно проверьте QR и public links с отдельного устройства;
6. только после этого создайте meetup tag/release и обновите слайды exact commit/tag.

Если во время прогона найден blocker — сначала исправьте template/demo/docs и повторите нужную часть прогона. Не переводите статус в verified по плану или ожидаемому результату.
