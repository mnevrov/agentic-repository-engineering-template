...
**Когда telemetry policy включена** (в частности, в greenfield/full profile), каждая завершённая попытка разработки — успешная, частичная, неуспешная или прерванная — добавляет одну строку JSON в `.ai/telemetry/cycles.jsonl`.

Журнал append-only:

- старые строки не редактируются и не переставляются;
- исправление оформляется новой записью с новым `cycle_id`;
- CI проверяет, что существующий префикс файла не изменён, если telemetry enforcement включён;
- CI требует новую telemetry row для содержательного изменения только в профиле, где эта policy включена.

Brownfield Stage 1 может явно оставить telemetry disabled/deferred; тогда эти recording/CI requirements не применяются до включения соответствующей policy.
...
В full profile и других профилях с включённой telemetry enforcement в CI выполняются две проверки относительно базового Git ref:
...
Первая запрещает переписывать историю, вторая запрещает содержательному изменению пройти без новой строки telemetry. Если telemetry policy ещё не включена (например, brownfield Stage 1), эти checks не являются обязательными.

## Когда записывать

Когда telemetry policy включена, запись телеметрии — часть рабочего цикла, а не факультативный отчёт:
